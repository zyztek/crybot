    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-5 gap-4">
          {chains.map(chain => (
            <div key={chain.name} className="bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-purple-500/20">
              <h3 className="text-white font-bold mb-4">{chain.name}</h3>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-slate-400">{text.health}</span>
                  <span className={`text-2xl font-bold ${chain.status === 'healthy' ? 'text-green-400' : 'text-yellow-400'}`}>
                    {chain.health}%
                  </span>
                </div>
                <div className="flex justify-between items-center">
