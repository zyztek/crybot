...
import { Shield, AlertTriangle, CheckCircle, XCircle, Eye, EyeOff, Search, RefreshCw } from 'lucide-react'
...
export default function WalletAudit() {
  ...
  const [auditResults, setAuditResults] = useState({
    ...
    issues: [
      {
        ...
        description: 'Your wallet has an unlimited allowance to Uniswap V3 Router. Consider limiting the amount.',
    ...
    permissions: [
      { dapp: 'Uniswap V3', allowance: 'Unlimited', spender: '0x68b3...C1a6', lastUsed: '2 days ago' },
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-red-600/20 to-orange-600/20 backdrop-blur-xl rounded-2xl border border-red-500/30 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-400">Analyze your wallet for security risks and vulnerabilities</p>
          </div>
          <div className="flex gap-3">
            <div className="bg-red-500/20 border border-red-500/50 rounded-xl px-4 py-2 text-red-400 font-semibold flex items-center gap-2">
              ...
              {auditResults.issues.filter(i => i.severity === 'critical' || i.severity === 'high').length} High Risk
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className={`bg-gradient-to-br ${auditResults.riskLevel === 'critical' ? 'from-red-600/20 to-red-900/20 border-red-500/30' : auditResults.riskLevel === 'high' ? 'from-orange-600/20 to-red-600/20 border-orange-500/30' : auditResults.riskLevel === 'medium' ? 'from-yellow-600/20 to-orange-600/20 border-yellow-500/30' : 'from-green-600/20 to-teal-600/20 border-green-500/30'} backdrop-blur-xl rounded-2xl border p-6`}>
          ...
          <div className={`text-lg mt-2 capitalize ${auditResults.riskLevel === 'critical' || auditResults.riskLevel === 'high' ? 'text-red-400' : auditResults.riskLevel === 'medium' ? 'text-yellow-400' : 'text-green-400'}`}>
        ...
        <div className="bg-gradient-to-br from-red-600/20 to-red-900/20 backdrop-blur-xl rounded-2xl border border-red-500/30 p-6">
          ...
          <div className="text-3xl font-bold text-white">{auditResults.issues.filter(i => i.severity === 'critical').length}</div>
        ...
        <div className="bg-gradient-to-br from-orange-600/20 to-yellow-600/20 backdrop-blur-xl rounded-2xl border border-orange-500/30 p-6">
          ...
          <div className="text-3xl font-bold text-white">{auditResults.issues.filter(i => i.severity === 'high' || i.severity === 'medium').length}</div>
                  ...
                  {issue.affectedAddresses.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {issue.affectedAddresses.map((addr, i) => (
                        <span key={i} className="bg-gray-800 px-2 py-1 rounded text-xs text-gray-400 font-mono">
                          {addr}
                        </span>
                      ))}
                    </div>
            ...
            <tbody className="text-white">
              {auditResults.permissions.map((perm, idx) => (
                <tr key={idx} className="border-b border-gray-700/50 hover:bg-gray-800/30">
                  ...
                  <td className="py-4 pr-4">
                    <span className={perm.allowance === 'Unlimited' ? 'text-red-400 font-semibold' : 'text-white'}>
                      {perm.allowance}
                    </span>
                  </td>
                  <td className="py-4 pr-4">
      ...
      <div className="bg-gradient-to-b from-gray-800/50 to-gray-900/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700/50">
            ...
            <p className="text-gray-400 text-sm">Store large amounts in hardware wallets like Ledger or Trezor for maximum security.</p>
          </div>
          <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700/50">
            <div className="text-green-400 font-semibold mb-2 flex items-center gap-2">
              <CheckCircle className="w-5 h-5" />
            ...
            <p className="text-gray-400 text-sm">Review and revoke unnecessary dapp permissions periodically.</p>
          </div>
          <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700/50">
            <div className="text-green-400 font-semibold mb-2 flex items-center gap-2">
              <CheckCircle className="w-5 h-5" />
            ...
            <p className="text-gray-400 text-sm">Always double-check URLs before connecting your wallet. Use official links.</p>
          </div>
          <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700/50">
            <div className="text-green-400 font-semibold mb-2 flex items-center gap-2">
              <CheckCircle className="w-5 h-5" />
            ...
            <p className="text-gray-400 text-sm">Enable two-factor authentication wherever possible for added protection.</p>
          </div>
        </div>
      </div>
    </div>
