...
import { Shield, AlertTriangle, CheckCircle, XCircle, Search, Globe, Link as LinkIcon, ExternalLink } from 'lucide-react';
  ...
  const handleScan = () => {
    ...
    setTimeout(() => {
      ...
      setResult({
        ...
        risks: isSafe ? [] : ['Unusual domain patterns', 'Newly registered domain', 'Missing SSL certificate', 'Poor reputation score'],
        positives: isSafe ? ['Valid SSL certificate', 'Domain age > 1 year', 'Good community feedback', 'No reported scams'] : ['Active user base'],
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {result && (
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700 overflow-hidden">
            {/* Result Header */}
            <div className={`p-6 ${result.isSafe ? 'bg-green-900/20' : 'bg-red-900/20'} flex items-center gap-4`}>
              {result.isSafe ? (
              ...
              <div>
                <h2 className={`text-2xl font-bold ${result.isSafe ? 'text-green-400' : 'text-red-400'}`}>
                  {result.isSafe ? 'Likely Safe' : 'Potentially Unsafe'}
                </h2>
                <p className="text-slate-400">
                  Confidence: {result.confidence.toFixed(1)}% | Safety Score: {result.finalScore.toFixed(0)}/100
                </p>
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="mt-6 pt-6 border-t border-slate-700">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`font-bold ${result.finalScore >= 70 ? 'text-green-400' : result.finalScore >= 40 ? 'text-yellow-400' : 'text-red-400'}`}>
                ...
                <div className="h-4 bg-slate-700 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-500 ${
                      result.finalScore >= 70 ? 'bg-gradient-to-r from-green-500 to-emerald-600' : 
                      result.finalScore >= 40 ? 'bg-gradient-to-r from-yellow-500 to-amber-600' : 
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { icon: Shield, title: 'Check SSL', text: 'Always look for HTTPS encryption before entering any faucet.' },
            { icon: AlertTriangle, title: 'Too Good to Be True', text: 'Be skeptical of faucets promising unrealistically high rewards.' },
            { icon: CheckCircle, title: 'Verify Reputation', text: 'Check community reviews and forums before using new faucets.' },
          ].map((tip, i) => (
