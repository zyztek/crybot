...
import { BarChart3, TrendingUp, DollarSign, Activity, Clock, ArrowUpRight, Users, ArrowDown, Zap, RefreshCw, Filter } from 'lucide-react'
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-purple-300">{text.lastUpdated}: {new Date().toLocaleString()}</p>
          </div>
        ...
        <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          {(Object.keys(metrics) as Array<keyof typeof metrics>).map(key => (
            <div key={key} className="bg-slate-800/50 backdrop-blur rounded-xl p-4 border border-purple-500/20">
          ...
          <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-4">
            {chainMetrics.map(chain => (
              <div key={chain.name} className="bg-slate-700/50 rounded-lg p-4">
                ...
                <span className={`text-sm ${chain.change.startsWith('+') ? 'text-green-400' : 'text-red-400'}`}>
                  {chain.change}
                </span>
              </div>
            ))}
