...
const AdvancedAnalytics = () => {
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredEarningsData.map(d => [d.date, d.faucets, d.staking, d.games, d.referrals, d.total]);
  ...
  const growthRate = filteredEarningsData.length > 1 
    ? ((filteredEarningsData[filteredEarningsData.length - 1].total - filteredEarningsData[0].total) / filteredEarningsData[0].total * 100).toFixed(1)
    : '0.0';
      ...
      {showDatePicker && (
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-xl p-6">
          ...
          <p className="text-slate-400 text-sm mt-3">
            Showing data from {customStartDate} to {customEndDate} ({filteredEarningsData.length} days)
          </p>
        </div>
      )}
        ...
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-gradient-to-br from-green-900/30 to-slate-900 border border-green-700/50 rounded-xl p-4">
              ...
              <div className="text-3xl font-bold text-green-400">${roiTotalReturn.toLocaleString(undefined, { maximumFractionDigits: 2 })}</div>
              <div className="text-sm text-green-300">
                +${roiProfit.toLocaleString(undefined, { maximumFractionDigits: 2 })} ({roiPercentage}%)
              </div>
            </div>
          </div>
          <div className="mt-4 p-3 bg-slate-800/50 rounded-lg text-sm text-slate-400">
            Based on average daily return rate of {roiDailyRate}%. This is a projection based on historical performance and does not guarantee future results.
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-xl p-4">
          ...
          <div className="text-3xl font-bold text-white">
            {Math.round(filteredEarningsData.reduce((sum, d) => sum + d.faucets, 0) / filteredEarningsData.length)}
          </div>
        ...
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-xl p-4">
          ...
          <div className="text-sm text-slate-400">
            {totalEarnings > 0 ? Math.round(filteredEarningsData.reduce((sum, d) => sum + d.games, 0) / totalEarnings * 100) : 0}% of total
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-xl p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {networkData.map((net) => (
                  <tr key={net.network} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                    ...
                    <td className="py-3 px-4 text-right text-green-400">${net.earnings.toLocaleString()}</td>
                    <td className="py-3 px-4 text-right text-purple-400">${net.avgClaim}</td>
                    <td className={`py-3 px-4 text-right ${net.trend.startsWith('+') ? 'text-green-400' : 'text-red-400'}`}>{net.trend}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            {activeCategory === 'faucets' && <span className="text-green-400 text-xs">✓ Active</span>}
          </div>
          <div className="text-2xl font-bold text-white">
            ${filteredEarningsData.reduce((sum, d) => sum + d.faucets, 0).toLocaleString()}
          </div>
          <div className="text-sm text-slate-400">
            {totalEarnings > 0 ? Math.round(filteredEarningsData.reduce((sum, d) => sum + d.faucets, 0) / totalEarnings * 100) : 0}% of total
          </div>
        </button>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            {activeCategory === 'staking' && <span className="text-yellow-400 text-xs">✓ Active</span>}
          </div>
          <div className="text-2xl font-bold text-white">
            ${filteredEarningsData.reduce((sum, d) => sum + d.staking, 0).toLocaleString()}
          </div>
          <div className="text-sm text-slate-400">
            {totalEarnings > 0 ? Math.round(filteredEarningsData.reduce((sum, d) => sum + d.staking, 0) / totalEarnings * 100) : 0}% of total
          </div>
        </button>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            {activeCategory === 'games' && <span className="text-purple-400 text-xs">✓ Active</span>}
          </div>
          <div className="text-2xl font-bold text-white">
            ${filteredEarningsData.reduce((sum, d) => sum + d.games, 0).toLocaleString()}
          </div>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            {activeCategory === 'referrals' && <span className="text-blue-400 text-xs">✓ Active</span>}
          </div>
          <div className="text-2xl font-bold text-white">
            ${filteredEarningsData.reduce((sum, d) => sum + d.referrals, 0).toLocaleString()}
          </div>
          <div className="text-sm text-slate-400">
            {totalEarnings > 0 ? Math.round(filteredEarningsData.reduce((sum, d) => sum + d.referrals, 0) / totalEarnings * 100) : 0}% of total
          </div>
        </button>
      </div>
      ...
      {activeCategory && (
        <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-4 text-center">
          <p className="text-purple-300">
            Showing filtered view for <span className="text-white font-bold capitalize">{activeCategory}</span>. 
            <button onClick={() => setActiveCategory(null)} className="text-purple-400 underline hover:text-white">
              Clear filter
            </button>
          </p>
        </div>
