...
import { FileText, Download, Share2, Eye, Clock, DollarSign, TrendingUp, Activity, ChevronRight, Plus, Calendar, Filter } from 'lucide-react'
...
export default function Reports() {
  ...
  const reports = [
    { id: 1, name: 'Daily Report', type: 'daily', createdAt: '2025-01-15', status: 'completed', rows: 1234, size: '2.3MB' },
    { id: 2, name: 'Weekly Summary', type: 'weekly', createdAt: '2025-01-14', status: 'completed', rows: 8923, size: '8.5MB' },
    { id: 3, name: 'Monthly Analysis', type: 'monthly', createdAt: '2025-01-01', status: 'completed', rows: 45678, size: '32.1MB' },
    { id: 4, name: 'Tax Report 2024', type: 'tax', createdAt: '2025-01-10', status: 'completed', rows: 56789, size: '45.2MB' },
    { id: 5, name: 'Portfolio Performance', type: 'portfolio', createdAt: '2025-01-13', status: 'scheduled', rows: 0, size: '-' },
    { id: 6, name: 'Trading Analysis Q4', type: 'trading', createdAt: '2025-01-12', status: 'in-progress', rows: 5678, size: '12.3MB' },
  ]
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-slate-800/50 backdrop-blur rounded-xl p-2 border border-purple-500/20 mb-6">
          <div className="flex gap-2 flex-wrap">
            {['overview', 'reports', 'templates'].map(tab => (
              ...
              >
                {tab === 'overview' ? text.overview : tab === 'reports' ? text.myReports : text.templates}
              </button>
            ))}
          </div>
        </div>
                  ...
                  <div key={report.id} className="bg-slate-700/50 rounded-lg p-4">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className={`text-xs px-2 py-1 rounded ${
                        ...
                        {report.status === 'completed' ? text.completed :
                         report.status === 'in-progress' ? text.inProgress : text.scheduled}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm text-slate-400">
                      <span>{report.rows} {text.rows}</span>
            ...
            <div className="bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-purple-500/20">
              ...
              <div className="grid grid-cols-2 gap-3">
                {templates.map((template, index) => (
                  <button key={index} className="bg-slate-700/50 hover:bg-slate-600/50 rounded-lg p-4 text-left transition-all">
                  ...
                  {reports.map(report => (
                    <tr key={report.id} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                      ...
                      <td className="py-4 flex gap-2">
                        <button className="text-purple-400 hover:text-purple-300 p-1" title={text.view}>
                          <Eye className="w-4 h-4" />
                        </button>
                        <button className="text-blue-400 hover:text-blue-300 p-1" title={text.download}>
                          <Download className="w-4 h-4" />
                        </button>
                        <button className="text-green-400 hover:text-green-300 p-1" title={text.share}>
        ...
        {activeTab === 'templates' && (
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {templates.map((template, index) => (
              <button key={index} className="bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-purple-500/20 hover:border-purple-500/50 transition-all text-left group">
