...
export default function FaucetsView({ faucets, onClaim, language, t, searchTerm = '', onClearSearch }: FaucetsViewProps) {
      ...
      <div className="flex gap-2 mb-4 flex-wrap items-center">
        <button 
          ...
          className={`px-3 py-1.5 rounded-lg font-medium transition-all text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 focus:ring-offset-slate-900 ${
            coinFilter === 'all' ? 'bg-purple-500 text-white' : 'bg-slate-800 text-purple-300 hover:bg-slate-700'
          }`}
        >
        ...
        <button 
          ...
          className={`px-3 py-1.5 rounded-lg font-medium transition-all text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 focus:ring-offset-slate-900 ${
            coinFilter === 'BTC' ? 'bg-orange-500 text-white' : 'bg-slate-800 text-orange-300 hover:bg-slate-700'
          }`}
        >
          BTC
        </button>
        <button 
          ...
          className={`px-3 py-1.5 rounded-lg font-medium transition-all text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 focus:ring-offset-slate-900 ${
            coinFilter === 'ETH' ? 'bg-blue-500 text-white' : 'bg-slate-800 text-blue-300 hover:bg-slate-700'
          }`}
        >
          ETH
        </button>
        <button 
          ...
          className={`px-3 py-1.5 rounded-lg font-medium transition-all text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 focus:ring-offset-slate-900 ${
            coinFilter === 'DOGE' ? 'bg-yellow-500 text-white' : 'bg-slate-800 text-yellow-300 hover:bg-slate-700'
          }`}
        >
          DOGE
        </button>
        <button 
          ...
          className={`px-3 py-1.5 rounded-lg font-medium transition-all text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 focus:ring-offset-slate-900 ${
            coinFilter === 'SOL' ? 'bg-purple-500 text-white' : 'bg-slate-800 text-purple-300 hover:bg-slate-700'
          }`}
        >
          SOL
        </button>
        <button 
          ...
          className={`px-3 py-1.5 rounded-lg font-medium transition-all text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 focus:ring-offset-slate-900 ${
            coinFilter === 'LTC' ? 'bg-gray-400 text-white' : 'bg-slate-800 text-gray-300 hover:bg-slate-700'
          }`}
        >
          LTC
        </button>
      ...
      {showFilters && (
        <div className="mb-4 p-4 bg-slate-800/50 border border-purple-500/20 rounded-xl">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-white font-medium">{language === 'es' ? 'Filtros avanzados' : 'Advanced Filters'}</h3>
            {activeFiltersCount > 0 && (
              <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Category filter */}
            <div>
              <label className="text-purple-300 text-sm mb-2 block">{language === 'es' ? 'Categoría' : 'Category'}</label>
              <select
            ...
            {/* Status filter */}
            <div>
              <label className="text-purple-300 text-sm mb-2 block">{language === 'es' ? 'Estado' : 'Status'}</label>
              <select
                value={statusFilter}
              ...
              >
                ...
                <option value="available">{language === 'es' ? '✅ Disponible' : '✅ Available'}</option>
                <option value="cooldown">{language === 'es' ? '⏳ En espera' : '⏳ Cooldown'}</option>
              </select>
            </div>
            
            {/* Difficulty filter */}
            <div>
              <label htmlFor="difficulty-filter" className="text-purple-300 text-sm mb-2 block">{language === 'es' ? 'Dificultad' : 'Difficulty'}</label>
              <select
