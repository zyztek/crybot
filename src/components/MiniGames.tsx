    ...
    <div className="space-y-6">
      <div className="flex justify-center">
        <div 
          ...
          style={{ 
            transform: spinning ? `rotate(${720 + (spinResult ?? 0) * 45}deg)` : `rotate(${(spinResult ?? 0) * 45}deg)`,
          }}
        >
          {wheelSegments.map((seg, i) => (
            ...
            >
              <span className="absolute top-17 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white font-bold">{seg.multiplier}x</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-4">
        ...
        {spinResult !== null && (
          <div className={`text-2xl font-bold ${games.spin.lastWin > 0 ? 'text-green-500' : 'text-red-500'}`}>
      ...
      <div className="bg-slate-800/50 rounded-xl p-4">
        ...
        <div className="flex gap-2">
          {games.spin.history.map((win, i) => (
            <div key={i} className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${win > 0 ? 'bg-green-500/20 text-green-500' : 'bg-red-500/20 text-red-500'}`}>
              {win > 0 ? '+' : '-'}
            </div>
          ))}
        </div>
  ...
  const renderCrash = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        {!crashed && (
          <div className="text-gray-400 mt-2">Cashout to win!</div>
        )}
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            { type: 'spin', icon: RotateCw, label: 'Ruleta', color: 'from-yellow-500 to-orange-500' },
            { type: 'dice', icon: Dice3, label: 'Dados', color: 'from-green-500 to-emerald-500' },
            { type: 'crash', icon: TrendingDown, label: 'Crash', color: 'from-red-500 to-pink-500' },
            ...
            >
              ...
              <div className="text-center text-sm opacity-75">{games[game.type as GameType].balance.toFixed(0)} BTC</div>
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mt-8">
          <div className="bg-slate-800/50 rounded-xl p-4 text-center">
            ...
            <div className="text-2xl font-bold text-white">
              {Object.values(games).reduce((sum, g) => sum + g.history.filter(w => w > 0).length, 0)}
            </div>
          ...
          <div className="bg-slate-800/50 rounded-xl p-4 text-center">
            ...
            <div className="text-2xl font-bold text-white">
              {Object.values(games).reduce((sum, g) => sum + g.lastWin, 0).toFixed(2)}
            </div>
          ...
          <div className="bg-slate-800/50 rounded-xl p-4 text-center">
            ...
            <div className="text-2xl font-bold text-white">
              {Object.values(games).reduce((sum, g) => sum + g.balance, 0).toFixed(0)}
            </div>
