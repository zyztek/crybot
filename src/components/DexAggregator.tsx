        ...
        <div className="lg:col-span-1 bg-gradient-to-b from-gray-800/50 to-gray-900/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="text-right text-gray-400 text-sm">
              ~${(parseFloat(fromAmount) * 1850).toFixed(2)} USD
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="text-right text-gray-400 text-sm">
              ~${((parseFloat(fromAmount) || 0) * quotes[0].price).toFixed(2)} USD
            </div>
          </div>
          ...
          <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700/50 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-white font-semibold">
                ${quotes[0].price.toFixed(2)} {toToken}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-400">Expected Output</span>
              <span className="text-green-400 font-semibold">
                {quotes[0].output.toFixed(2)} {toToken}
              </span>
            </div>
            <div className="flex justify-between text-sm">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span
                className={`font-semibold ${quotes[0].priceImpact < 0.1 ? 'text-green-400' : quotes[0].priceImpact < 0.5 ? 'text-yellow-400' : 'text-red-400'}`}
              >
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-white">
                      {quote.output.toFixed(2)} {toToken}
                    </div>
                ...
                <div className="mt-4 flex items-center justify-between">
                    ...
                    {quote.route.map((token, i) => (
                      <span key={i} className="flex items-center">
                        ...
                        {i < quote.route.length - 1 && (
                          <ArrowLeftRight className="w-4 h-4 text-gray-500 mx-1" />
                        )}
                      </span>
                    ))}
                  </div>
                  <div className="flex items-center gap-4 text-sm">
                    <span
                      className={`font-semibold ${quote.priceImpact < 0.1 ? 'text-green-400' : quote.priceImpact < 0.5 ? 'text-yellow-400' : 'text-red-400'}`}
                    >
                      Impact: {quote.priceImpact.toFixed(2)}%
                    </span>
                    <span className="text-gray-400">
                      Gas:{' '}
                      <span className="text-white font-semibold">${quote.gasUsd.toFixed(2)}</span>
                    </span>
