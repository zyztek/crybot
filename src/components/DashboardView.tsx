              ...
              <div key={item.coin} className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-purple-300 flex items-center gap-2">
                    <span className="w-6 h-6 bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded flex items-center justify-center text-xs">
                      {item.icon}
                    </span>
                    {item.coin}
                  </span>
        ...
        <div className="bg-slate-800/50 border border-purple-500/20 rounded-xl p-5 backdrop-blur-sm">
          ...
          <div className="space-y-3">
            {achievements.slice(0, 4).map((achievement: Achievement) => (
              <div
                key={achievement.id}
                className={`flex items-center gap-3 p-2 rounded-lg ${achievement.unlocked ? 'bg-green-500/10' : 'bg-slate-900/50'}`}
              >
