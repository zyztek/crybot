...
describe('CryptoStore - Edge Cases', () => {
  ...
  it('handles claimFaucet with different difficulty levels', () => {
    ...
    if (easyFaucet) {
      ...
      expect(useCryptoStore.getState().faucets.find(f => f.id === easyFaucet.id)?.status).toBe(
        'wait'
      );
    }
    ...
    if (mediumFaucet) {
      ...
      expect(useCryptoStore.getState().faucets.find(f => f.id === mediumFaucet.id)?.status).toBe(
        'wait'
      );
    }
  });
...
describe('CryptoStore - Persist Hydration', () => {
  ...
  it('wallet balance structure matches persisted type', () => {
    ...
    expect(state.walletBalance).toEqual(
      expect.objectContaining({
