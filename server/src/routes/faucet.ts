...
router.get('/:coin', asyncHandler(async (req, res) => {
...
router.post('/claim', authenticate, asyncHandler(async (req: AuthRequest, res) => {
  ...
  if (lastClaim) {
    const nextClaimTime = new Date(lastClaim.claimedAt.getTime() + faucet.intervalHours * 60 * 60 * 1000);
...
router.get('/status/:coin', authenticate, asyncHandler(async (req: AuthRequest, res) => {
  const coinParam = req.params.coin;
  if (!coinParam || Array.isArray(coinParam)) {
    return res.status(400).json({ success: false, error: 'Invalid coin parameter' });
  }
