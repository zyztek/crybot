...
const generateTokens = (userId: string, sessionId: string) => {
  const accessToken = jwt.sign({ userId, sessionId }, jwtConfig.secret, {
    expiresIn: jwtConfig.expiresIn,
  } as jwt.SignOptions);

  const refreshToken = jwt.sign({ userId, sessionId, type: 'refresh' }, jwtConfig.secret, {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const payload = jwt.verify(refreshToken, jwtConfig.secret) as {
      userId: string;
      sessionId: string;
      type: string;
    };
