...
router.get('/overview', authenticate, asyncHandler(async (req: AuthRequest, res) => {
...
router.get('/daily', authenticate, asyncHandler(async (req: AuthRequest, res) => {
...
router.get('/user', authenticate, asyncHandler(async (req: AuthRequest, res) => {
  ...
  const [
    ...
    prisma.transaction.findMany({
      where: { userId: req.user!.id, type: { in: ['claim', 'referral_bonus', 'achievement_reward'] } },
      select: { amount: true },
    }),
