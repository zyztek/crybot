...
import {
  ...
  type TestnetChain,
  type Persona,
} from '../services/testnetOrchestrator.js';
...
const createPersonaSchema = z.object({
  ...
  chains: z
    .array(z.enum(['sepolia', 'holesky', 'goerli', 'solana', 'bitcoin-testnet', 'bsc-testnet']))
    .min(1),
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    const amount =
      data.amount || testnetOrchestrator.getChainConfig(data.chain)?.nativeSymbol || '0.01';
    ...
    res.json({
      ...
      data: {
        ...
        explorer: result.txHash
          ? `${testnetOrchestrator.getChainConfig(data.chain)?.explorer}/tx/${result.txHash}`
          : null,
      },
      error: result.error,
    });
  ...
  asyncHandler(async (req, res) => {
    res.json({
      ...
      data: {
        ...
        configuredChains: testnetOrchestrator
          .getSupportedChains()
          .filter(chain => testnetOrchestrator.getChainConfig(chain)?.rpcUrl),
      },
    });
  })
);
